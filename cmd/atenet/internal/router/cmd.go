...
func NewRouterCmd() *cobra.Command {
	...
	cmd.Flags().DurationVar(&cfg.ParkedRequest.Budget, "parked-request-budget", ingress.DefaultParkedRequestBudget, fmt.Sprintf("Maximum time a resume flight keeps starting resume attempts for a parked request; concurrent requests for the same actor share one flight and its attempts. An attempt still in flight at the budget is never cancelled: each caller waits up to a further %s for it before receiving a retryable 503, so the worst-case hold on a request is this budget + %s", ingress.CommittedAttemptWait, ingress.CommittedAttemptWait))
	...
	cmd.Flags().DurationVar(&cfg.DrainTimeout, "drain-timeout", 0, fmt.Sprintf("Deadline for the ext_proc drain on shutdown; streams still open past it (parked requests included) are forcefully cancelled. 0 (the default) derives the worst-case resume hold (the mode's resume budget + %s: --parked-request-budget with parking on, 15s fail-fast otherwise) + the actor route timeout + margin so held requests always finish normally. Explicit values must be >= that worst-case hold", ingress.CommittedAttemptWait))
