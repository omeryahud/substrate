...
func TestActorResumer_Parking(t *testing.T) {
	...
	t.Run("InFlightAttemptRunsToCompletion", func(t *testing.T) {
		synctest.Test(t, func(t *testing.T) {
			...
			// elapses is NEVER cancelled — ateapi has already claimed a worker
			// for it, and cancelling would discard the restore and strand that
			...
			if ctxErrAtReturn != nil {
				t.Errorf("the in-flight attempt's context was cancelled (%v); the budget must never cancel an attempt", ctxErrAtReturn)
			}
	...
	t.Run("LateRetryableErrorIsBudgetExhaustion", func(t *testing.T) {
		synctest.Test(t, func(t *testing.T) {
			...
			_, _, err := resumer.ResumeActor(context.Background(), testActorRef)
			if got := status.Code(err); got != codes.FailedPrecondition {
				t.Errorf("expected FailedPrecondition after a late retryable failure, got %v (err=%v)", got, err)
			}
